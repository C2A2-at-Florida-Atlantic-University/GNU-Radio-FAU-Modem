...
Status: **feature-complete as designed (2026-09-15), not yet run against
real hardware.** Everything below this line is the original design record;
see the "Implementation status" sections immediately below for what
actually exists in `scripts/fau_deployer/` today and how it deviates from
(or confirms) this doc. Companion to the root `CLAUDE.md` (which covers
