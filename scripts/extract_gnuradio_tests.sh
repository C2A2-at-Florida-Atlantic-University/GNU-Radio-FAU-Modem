#!/bin/bash
#
# extract_gnuradio_test.sh
# TEST-BUILD variant of the headless GNU Radio extractor.
#
# Difference from the production script (extract_gnuradio.sh):
#   - Adds the *-ptest test packages (gnuradio-ptest, volk-ptest) and the
#     ptest-runner so the payload can be exercised on-target with the upstream
#     QA/regression tests. These are NOT shipped in production.
#   - The wrong-ABI guard is SOFTENED to a loud warning instead of a hard
...
#         gnuradio-ptest \
#     "
# (Each append needs a LEADING SPACE; BitBake concatenates :append literally.)
#
# Behavior:
#   - Iterates over both boards; missing deploy dir / RPMs -> WARNING, continue.
#   - Resolves RPM filenames by glob (survives git-hash / release bumps).
#   - cpython-312 extensions emitted directly; no 3.10->3.12 migration. Bindings
#     build because the gnuradio bbappend sets -DPYBIND11_FINDPYTHON=ON (+ target
#     pybind11_DIR, -DENABLE_PYTHON=ON), avoiding the legacy FindPythonLibsNew
#     32-bit-target / 64-bit-native cross abort.
#
set -u

# Set TEST_STRICT=1 to restore the production hard-stop on wrong-ABI .so.
TEST_STRICT="${TEST_STRICT:-0}"
...
# downgraded to NOTE (a test box may not have built every -ptest package).
TESTPKGS=(
    "ptest-runner"      # the harness that executes installed ptests
    "gnuradio-ptest"    # gnuradio's own test suite
    "volk-ptest"        # volk kernel tests (SIMD correctness)
)
...
extract_board() {
    ...
    for dep in "${TESTPKGS[@]}"; do
        if extract_one_pkg "${dep}" "${rpm_dir}" "${stage}"; then
            ...
            echo "  NOTE: test package '${dep}' not found (enable it in the test" >&2
            echo "        image's IMAGE_INSTALL if you need it). Continuing." >&2
        fi
    done
    ...
    if [ -n "${bad}" ]; then
        ...
        if [ "${TEST_STRICT}" = "1" ]; then
            echo "  TEST_STRICT=1: refusing to package ${board}." >&2
            return
        fi
        # Test mode default: package anyway for inspection, but loudly flagged.
        echo "  WARNING (test mode): packaging ${board} despite wrong-ABI .so so" >&2
        echo "           the tree can be inspected. Do NOT deploy this to a modem." >&2
    fi
