...
# Stands in for grcc; see tests/test_generate.py for why it is a real
# subprocess rather than a mock.
GRCC_STUB = """#!/usr/bin/env python3
import os, re, sys
argv = sys.argv[1:]
out = argv[argv.index("-o") + 1]
src = argv[-1]
fg_id = re.search(r"^\\s*id: (\\S+)$", open(src).read(), re.M).group(1)
...
class TestProcessPhase(unittest.TestCase):
    """--flowgraph accepting a .grc. grcc is stubbed (see
    test_generate.py's reasoning): what is under test here is that the CLI
    routes a .grc through Process, rewrites the flowgraph it goes on to
    deploy, and opens no port to do it."""

    def setUp(self):
        if not fx.HAVE_YAML:
            self.skipTest("PyYAML not installed")
        self.tmp = tempfile.mkdtemp(prefix="fau_cli_process_")
        self.addCleanup(shutil.rmtree, self.tmp, ignore_errors=True)
        self.build = os.path.join(self.tmp, "build")
        self.grc = fx.write(fx.rx_with_gui(), self.tmp)
        self.grcc = os.path.join(self.tmp, "grcc_stub")
    ...
    def _argv(self, *extra):
        return ["--flowgraph", self.grc, "--build-dir", self.build,
                "--grcc", self.grcc] + list(extra)

    def test_process_only_compiles_and_needs_no_port(self):
        # Compiling is local; requiring a port would mean plugging a board
        # in to do code generation.
        self.assertEqual(cli.main(self._argv("--process-only")), cli.EXIT_OK)
        self.assertTrue(os.path.isfile(os.path.join(self.build, "rx_demo.py")))

    def test_transform_only_writes_the_headless_copy_and_stops(self):
        cli.main(self._argv("--process-only", "--transform-only"))
        self.assertTrue(os.path.isfile(
            os.path.join(self.build, "rx_demo.headless.grc")))
        self.assertFalse(os.path.isfile(os.path.join(self.build, "rx_demo.py")))

    def test_a_grc_is_compiled_before_the_payload_is_built(self):
        # --dry-run stops before the port, so reaching EXIT_OK means the
        # payload was assembled from the generated .py.
        self.assertEqual(cli.main(self._argv("--dry-run")), cli.EXIT_OK)
        self.assertTrue(os.path.isfile(os.path.join(self.build, "rx_demo.py")))

    def test_no_process_refuses_a_grc_rather_than_compiling_it(self):
        with self.assertRaises(SystemExit) as cm:
            cli.main(self._argv("--no-process", "--dry-run"))
        self.assertEqual(cm.exception.code, cli.EXIT_USAGE)

    def test_a_mode_that_disagrees_with_the_flowgraph_is_refused(self):
        with self.assertRaises(SystemExit) as cm:
            cli.main(self._argv("--process-only", "--mode", "tx"))
        self.assertEqual(cm.exception.code, cli.EXIT_USAGE)

    def test_message_controls_refuse_until_allowed(self):
        grc = fx.write(fx.doc([fx.fau_source(), fx.msg_button()],
                              fg_id="msgs"), self.tmp)
        argv = ["--flowgraph", grc, "--build-dir", self.build,
                "--grcc", self.grcc, "--process-only"]
        with self.assertRaises(SystemExit) as cm:
            cli.main(argv)
        self.assertEqual(cm.exception.code, cli.EXIT_USAGE)
        self.assertEqual(cli.main(argv + ["--allow-message-controls"]),
                         cli.EXIT_OK)

    def test_processing_rewrites_what_gets_deployed(self):
        # Everything downstream -- payload assembly, --params checking, the
        # name the board runs -- works on the generated Python and never
        # has to know a .grc was involved. search_dirs is how a sibling
        # module left behind beside the .grc still gets shipped.
        parser = cli._build_parser()
        args = parser.parse_args(self._argv())
        cli._process(args)
        self.assertEqual(args.flowgraph, os.path.join(self.build, "rx_demo.py"))
        self.assertEqual(tuple(args.search_dirs), (os.path.dirname(self.grc),))
        self.assertEqual(args.generated.target, "rx")

    def test_a_py_flowgraph_passes_straight_through(self):
        parser = cli._build_parser()
        args = parser.parse_args(["--flowgraph", TX_SINE])
        cli._process(args)
        self.assertEqual(args.flowgraph, TX_SINE)
        self.assertEqual(tuple(args.search_dirs), ())
        self.assertIsNone(args.generated)


class TestProcessArgumentErrors(unittest.TestCase):
    def _usage(self, argv):
        with self.assertRaises(SystemExit) as cm:
            cli.main(argv)
        self.assertEqual(cm.exception.code, cli.EXIT_USAGE)

    def test_watch_without_process_only_is_refused(self):
        # A watch that redeployed on every save would ship a flowgraph to a
        # board nobody is looking at.
        self._usage(["--flowgraph", "x.grc", "--watch", "--port", "/dev/x"])

    def test_process_only_and_no_process_contradict(self):
        self._usage(["--flowgraph", "x.grc", "--process-only", "--no-process"])

    def test_process_only_on_a_py_says_there_is_nothing_to_do(self):
        self._usage(["--flowgraph", TX_SINE, "--process-only"])
